...
#
# ---------------------------------------------------------------------------
# PER MODEL, because the threshold belongs to the EMBEDDER, not to the harness.
#
# Different models put their similarities on different scales. Measured on the
# same known-true pairs (a golden passage against the chunk that literally
# contains it):
#
#   qwen/qwen3-embedding-8b        median 0.492
#   perplexity/pplx-embed-v1-0.6b  median 0.300
#
# Against a single hardcoded 0.45, qwen cleared 8 of 12 and pplx cleared 1.
# Read naively that says pplx cannot retrieve. It actually says the ruler was
# built for a different model — and that is how this repo nearly concluded a
# perfectly good embedder was catastrophic, in the same week it had already
# been caught reporting four in five true containments as misses.
#
# eval/thresholds.json holds the derived value per model and is regenerated by
# `python -m eval.calibrate --model <m> --write`. The fallback below applies to
# any model that has not been calibrated yet, and is deliberately qwen's
# historical value so behaviour is unchanged for anything already measured.
FALLBACK_MATCH_THRESHOLD = 0.45
_THRESHOLDS_PATH = Path(__file__).resolve().parent / "thresholds.json"
...
def match_threshold(embedding_model: str | None = None) -> float:
    """The calibrated cosine threshold for one embedding model.

    Read at CALL time. A module-level constant would freeze whichever model was
    configured when the process booted, which is exactly the class of bug
    CLAUDE.md warns about for judges.JUDGE_MODEL.
    """
    if embedding_model is None:
        try:
            ...
            return FALLBACK_MATCH_THRESHOLD
    entry = _load_thresholds().get(embedding_model)
    if isinstance(entry, dict) and isinstance(entry.get("threshold"), (int, float)):
        return float(entry["threshold"])
    return FALLBACK_MATCH_THRESHOLD


# Kept as a name because tests and comments reference it, but it is now the
# CONFIGURED model's threshold rather than a constant for all of them.
...
def _chunk_matches_passage(
    chunk_emb: list[float],
    passage_emb: list[float],
    threshold: float | None = None,
) -> bool:
    return cosine(chunk_emb, passage_emb) >= (
        MATCH_THRESHOLD if threshold is None else threshold
    )
...
def context_recall(retrieved_embs: list[list[float]], golden_embs: list[list[float]],
    threshold: float | None = None,
) -> float:
    ...
    for ge in golden_embs:
        if any(_chunk_matches_passage(ce, ge, threshold) for ce in retrieved_embs):
            hits += 1
...
def precision_at_k(
    ...
    rel = 0
    for ce in top:
        if any(_chunk_matches_passage(ce, ge, threshold) for ge in golden_embs):
            rel += 1
    return rel / len(top)
...
def hit_rate_at_k(
    ...
    return int(any(_chunk_matches_passage(ce, ge, threshold)
